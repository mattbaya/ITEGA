...
= 0.2.8 =
* A site that loses its credentials re-certifies itself instead of silently filing nothing
* A daily check notices a key the exchange no longer holds
* The admin says so, since the failure is invisible from the outside
...
* The reader is linked to somewhere a person can act, not to an OIDC issuer
* Both screens name the organization that decided, or that could not be reached
* Gate copy cut by about half
