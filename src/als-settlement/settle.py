...
Reads the access_events hypertable for a given period, computes debits
(home bases) and credits (publishers), deducts the ITEGA governance fee,
and writes the results to the als_settlement database plus CSV/JSON reports.
...
def dec(value: Any) -> Decimal:
    """Convert a value to Decimal with 4 decimal places."""
...
def fetch_aggregates(
    ...
    with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
        # Home base debits: one row per home_base_id
        cur.execute(
            """
            SELECT home_base_id,
        ...
        # Publisher credits: one row per pub_mbr_id
        cur.execute(
            """
            SELECT pub_mbr_id,
                   COUNT(*)::int                          AS total_events,
...
def validate_balance(
    ...
    """
    Verify that the sum of home-base debits equals the sum of publisher
    credits.  This is a fundamental accounting invariant.
    """
